...
module.exports = function(ctx, opts, callback) {
    ...
    var queryString = _.chain(queryParam.split(' ')).map(function(word) {
        return word + '~';
    }).value().join(' AND ');
    ...
    var elasticQuery = SearchUtil.createQuery(query, filter, opts);

    // Override the default sorting
    elasticQuery.sort = [{'sort': {'order': 'asc'}}];
    elasticQuery.min_score = 1;

    // Wrap the query into the top-level Query DSL "query" object
    return callback(null, elasticQuery);
};
